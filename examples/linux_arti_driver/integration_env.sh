...
arti_load_integration_config() {
    ...
    while IFS=$'\t' read -r key value; do
        case "$key" in
            ARTI_RTL_TOP|ARTI_RTL_SOURCE|ARTI_MMIO_BASE|ARTI_DT_COMPAT|ARTI_IRQ_BASE|ARTI_DISPLAY|ARTI_DISPLAY_WIDTH|ARTI_DISPLAY_HEIGHT|ARTI_DISPLAY_FORMAT|ARTI_DISPLAY_FB_OFFSET|ARTI_DISPLAY_FB_SIZE|DRIVER_KO|DRIVER_DEPS|DRIVER_MARKER|SKIP_GENERIC_TEST|GPU_REFERENCE)

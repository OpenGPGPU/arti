...
arti_load_integration_config() {
    ...
    while IFS=$'\t' read -r key value; do
        case "$key" in
            ARTI_RTL_TOP|ARTI_RTL_SOURCE|ARTI_MMIO_BASE|ARTI_DT_COMPAT|ARTI_IRQ_BASE|ARTI_DISPLAY|ARTI_DISPLAY_WIDTH|ARTI_DISPLAY_HEIGHT|ARTI_DISPLAY_FORMAT|ARTI_DISPLAY_SOURCE|ARTI_DISPLAY_FB_OFFSET|ARTI_DISPLAY_FB_SIZE|ARTI_DISPLAY_ADDR_REG|ARTI_DISPLAY_STRIDE_REG|ARTI_DISPLAY_CTRL_REG|ARTI_DISPLAY_WIDTH_REG|ARTI_DISPLAY_HEIGHT_REG|ARTI_DISPLAY_REFRESH_HZ|DRIVER_KO|DRIVER_DEPS|DRIVER_MANIFEST|DRIVER_MARKER|SKIP_GENERIC_TEST|GPU_REFERENCE)
